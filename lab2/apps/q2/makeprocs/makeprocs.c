...
void main (int argc, char *argv[])
{
  ...
  for(i=0; i<numprocs; i++) {
    process_create(FILENAME_PRODUCER, h_mem_str, s_procs_completed_str, general_lock_str,  NULL);
    Printf("Process %d producer created\n", i);
  }
  for(i=0; i<numprocs; i++) {
    process_create(FILENAME_CONSUMER, h_mem_str, s_procs_completed_str, general_lock_str, NULL);
