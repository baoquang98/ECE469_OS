...
int CondWait (Cond *cond) {
  ...
  //AccquireLock(cond->lock);
 // check is the lock has been aquired
 //if (cond->lock.pid != -1) return SYNC_FAIL;

  dbprintf ('I', "CondWait: Old interrupt value was 0x%x.\n", intrval);
  dbprintf ('s', "CondWait: Proc %d waiting on condition variable number %d \n", GetCurrentPid(), (int)(cond-conds));
